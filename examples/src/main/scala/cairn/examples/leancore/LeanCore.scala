package cairn.examples.leancore
...
/** LeanCore — the formal-methods IR ladder's rung past MiniTT (§5b, §2c
  ...
  */
object LeanCore:
  lazy val fragments: List[Fragment] = PackLoader.requireOwn("leancore")

  def ownCompose: Either[List[ComposeError], ComposedLanguage] =
    Compose.compose("leancore", fragments)

  lazy val language: ComposedLanguage = PackLoader.requireClosed("leancore")
