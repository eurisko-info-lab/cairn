...
| BFT finality | Classic `n=3f+1`. `seq` = height. Messages bind domain `cairn-bft-v1`, **chainId** (genesis digest), and `replicaSet`. View-change broadcasts local VC, self-delivers NewView, and tolerates `f` unreachable peers via **concurrent** quorum fan-out with per-request deadlines. Prepared claims require signed PrePrepare from the **designated primary** plus prepare-quorum seals (persisted). NewView is a certificate of sealed ViewChange envelopes. Propose/view-change requests are epoch-bound. Follower adoption journals embed verified certificate **bodies** and resume before checkpoint verification on gossip/CLI/pull paths. Hot-reload adopts a successor set when tip height crosses activation. **Still open:** replica-local pacemaker / view-status protocol (client-driven view progression remains); ledger-anchored membership reconfiguration (history is crypto-validated but operator-distributed). |
