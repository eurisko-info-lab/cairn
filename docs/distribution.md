...
| BFT finality | Classic `n=3f+1`. `seq` = height. Messages bind domain `cairn-bft-v1`, **chainId** (genesis digest), and `replicaSet`. Cross-view safety uses an explicit per-sequence **prepared lock** (highest prepared view), installed on local prepare quorum and on accepting NewView; conflicting later PrePrepares are rejected (slot scan uses `maxBy`, never `collectFirst`). View-change is successor-only (`view+1`) and rate-limited on each replica; remote one-signer `ViewChangeRequest` cannot jump arbitrarily high. NewView is a certificate of sealed ViewChange envelopes. Propose/view-change requests are epoch-bound. Follower adoption journals are treated as untrusted: CAS load → ledger replay → checkpoint extension → certificate verification, then mutate. Concurrent cert polling with backoff after view-change. Hot-reload adopts a successor set when tip height crosses activation. Each replica tracks primary activity and refuses a remote `/bft/view-change` trigger until its own `PrimaryTimeoutMs` has elapsed since the last observed PrePrepare/NewView (`requestViewChangeIfTimedOut`); `/bft/status` serves a signed `ViewStatus` so peers can confirm real convergence instead of guessing from HTTP outcomes. Replica-set transitions are ledger-anchored: `BftFinality.recordReplicaSetTransition` records a predecessor-approved manifest's digest via `Tx.RecordCertificate`, and `verifyReplicaSetHistoryAnchored`/`refreshHistory` refuse to hot-reload a transition that isn't recorded in this node's own replayed ledger state (`bft replica-set anchor <signer>` CLI). |
