# Migration history (archaeology)

Chronological notes from the Kernel/Core/System/User refactor and the
post-migration authority work. **Current** boundaries live in
[architecture.md](architecture.md). The operational plan remains
[MIGRATION-PLAN.md](../MIGRATION-PLAN.md).

This file preserves intermediate designs, rejected approaches, and
phase-by-phase narrative that previously mixed into `architecture.md`.
...
Post-migration priority #1: make effect interfaces Meta-defined — generate typed
rights and resource vocabularies instead of hand-maintaining them independently.
...
**Explicit injection — DONE (final state).** Language packs are classes taking
...
**`AuthorityGate`: per-family registry (intermediate — later superseded)** —
resolves the "per-family Enforce granularity" gap named above via
`AuthorityGate.forFamily(family: Effects.Family): AuthorityGate`, a
lazily-bootstrapped registry replacing the single shared `default`. Each
family now gets its own instance, so
...
done at this stage**: the cost (rewriting test suites that don't exercise
authority at all) didn't match the benefit over this registry. Available as
separate future work if still wanted.
...
These appeared in the live architecture doc at one point and are **not**
current:

| Design | What it was | Fate |
| ------ | ----------- | ---- |
| Exact-name `actionsOf` (1 Action per Request ctor) | Inferred rights from constructor names | Replaced by explicit `requestActions` many-to-one map |
| Vestigial `Http` / `Network` / `Crypto` interface families | Request/Response enums with no handlers | Removed; real I/O lives elsewhere |
| Unused `LedgerTransport` Request/Response types + dead sibling actions | Parallel unused contract beside live `LedgerAppend` | Types/actions removed; `LedgerAppend` kept |
| Handler convenience methods as public entry points | Bypass of `perform` / authorization | Privatized or deleted; `perform` sole entry |
| `PackFiles` alias over `Workspace` | Compatibility re-export | Removed after PackLoader migration |
| Ambient `AuthorityGate` singleton (`@volatile` mode/policies) | Process-global mutable gate | Became instantiable class, then fully injected |
| `AuthorityGate.default` shared Enforce instance | One gate for all families | Superseded by `forFamily`, then by explicit injection |
| `AuthorityGate.forFamily` registry | Lazy per-family gates without full DI | **Superseded** — no ambient registry; roots pass gates |
| Reflective `PackAccess` `Class.forName(PackLoader$)` bootstrap | Force install without compile dep | Restored briefly for ordering safety, then **removed** with explicit `PackAccess` injection |
| `PackAccess.get` / `install` / `withInstalled` | Process-global pack registry | **Removed** — packs take `PackAccess` as constructor arg |
| Allow-all Enforce bootstrap (`Subject("*")` × all actions) | Prove Enforce path runs | Kept only for non-PackLoader roots; PackLoader uses narrow workspace policy |
| Handler-invented `Subject("local")` | Placeholder identity inside handlers | Moved to `EffectContext` at composition roots |

See [architecture.md](architecture.md) for what replaced these.
