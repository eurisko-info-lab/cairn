...
| GRANITE | Language Studio | Native Cairn implementation — languages, change capabilities, validation models, migrations, foreign surfaces, and Studio profiles are one digest-bound project changed through ΔMeta; grammar assets are isolated to ΔGrammar; mixed proposals retain both replay witnesses and validate the complete target-revision graph atomically |
| GRANITE | Proof and Projection Studio | Native Cairn implementation — proof goals feed untrusted search into the independent kernel checker, producing linked derivation/proof-term/theorem/certificate artifacts; Rosetta host files are content-addressed projection evidence with explicit obligations, while LeanCore and HVM/IC agreement certificates remain separately envelope-checked |
| GRANITE | Artifact-only application startup | Native Cairn implementation — an `ApplicationManifest` root discovers language fragments, grammars, capability selections, extensions, and typed entries recursively; installation verifies every digest and startup reconstructs the full runtime from CAS without receiving host-built language or entry maps |
| GRANITE | Pack and application ecosystem | Native Cairn implementation — domain-separated Ed25519 release bundles bind namespace, semantic version, pack/application root, migrations, ancestry, and publisher key; registries enforce namespace ownership, revocation, ledger publication, and anti-equivocation before version discovery or migration routing; replication reuses the artifact dependency installer |
